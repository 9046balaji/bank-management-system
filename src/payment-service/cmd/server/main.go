...
func main() {
	...
	if err != nil {
		log.Fatalf("Failed to open payments_db connection: %v", err)
	}
	defer db.Close()
...
func (s *Server) handleStartup(w http.ResponseWriter, r *http.Request) {
	...
	json.NewEncoder(w).Encode(map[string]string{"status": "ok", "service": "payment-service"})
}
...
func (s *Server) handleReady(w http.ResponseWriter, r *http.Request) {
	...
	w.Header().Set("Content-Type", "application/json")
	w.WriteHeader(http.StatusOK)
	json.NewEncoder(w).Encode(map[string]string{"status": "ready", "database": "payments_db"})
}
