...
const Sidebar: React.FC<SidebarProps> = memo(function Sidebar({ currentView, setView, role }) {
  ...
  const handleItemClick = useCallback((viewId: View) => {
    setView(viewId);
  }, [setView]);

  return (
    <aside 
      className="w-full md:w-64 bg-surface-light dark:bg-surface-dark border-r border-slate-200 dark:border-slate-800 flex flex-col shrink-0"
      role="navigation"
